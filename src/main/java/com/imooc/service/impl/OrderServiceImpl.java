...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDTO create(OrderDTO orderDTO) {
        ...
        String orderId = KeyUtil.genUniqueKey();    // 生成随机订单
    ...
    public OrderDTO findOne(String orderId) {
        ...
        OrderMaster orderMaster = orderMasterMapper.findById(orderId).orElse(null);
        if (orderMaster == null) {
    ...
    public OrderDTO cancel(OrderDTO orderDTO) {
        ...
        if (CollectionUtils.isEmpty(orderDTO.getOrderDetailList())) {
            log.error("【取消订单】订单中无商品更新，orderDTO={}", orderDTO);
        ...
        List<CarDTO> carDTOList = orderDTO.getOrderDetailList().stream().map(e ->
                new CarDTO(e.getProductId(), e.getProductQuantity()))
                .collect(Collectors.toList());
        productService.increseStock(carDTOList);
        // 如果已支付，需要退款
    ...
    public OrderDTO paid(OrderDTO orderDTO) {
        ...
        OrderMaster orderMaster = new OrderMaster();
        BeanUtils.copyProperties(orderDTO,orderMaster);
        OrderMaster updateReault = orderMasterMapper.save(orderMaster);
        if (updateReault == null) {
